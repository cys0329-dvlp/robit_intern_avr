...
// 최종 정지 상태
//
// ID5-ID7 좌표 일치 후 그리퍼 오픈 -> 후진 -> HOME 복귀까지
// 전부 끝난 뒤에 1
// ============================================================
...
// return_rotation_active/finished : HOME 방향 회전 상태
// return_forward_active/finished  : HOME 좌표까지 직진 상태
// ============================================================

uint16_t home_x = 0;
uint16_t home_y = 0;
uint8_t home_saved = 0;

uint8_t returning_home = 0;

uint8_t return_rotation_active = 0;
uint8_t return_rotation_finished = 0;

uint8_t return_forward_active = 0;
uint8_t return_forward_finished = 0;
...
// 임의의 두 좌표 사이 각도
//
// CalculateAngleIDs와 동일한 정의(위=0°, 오른쪽=+90° ...)를
// ID가 아닌 임의의 (x, y) 좌표쌍에 대해 그대로 사용하기 위한
// 버전. HOME 복귀 각도 계산에 사용한다.
// ============================================================

int16_t CalculateAngleXY(
uint16_t x1,
uint16_t y1,
uint16_t x2,
uint16_t y2
...
// HOME 복귀 전용
//
// 현재 ID5 좌표가 임의의 목표 좌표(target_x, target_y)에
// 도달했는지 판정한다. ID5_ReachedTarget7과 동일한 방식이며
// HOME 좌표 비교에 재사용하기 위해 목표 좌표를 파라미터로 받는다.
// ============================================================

uint8_t ID5_ReachedPosition(
uint16_t target_x,
uint16_t target_y
)
{
...
void StartRotation(void)
{
	...
	// 왼쪽 회전 보정
	//
	// 실측 결과 왼쪽 회전이 목표각보다 더 많이 돌아가는
	// 경향이 있어서, id_angle이 음수(왼쪽 방향)일 때만
	// 목표각을 30도만큼 줄여서(0쪽으로 당겨서) 설정한다.
	// 오른쪽(id_angle >= 0) 회전은 그대로 유지한다.
	// ----------------------------------------------------
...
// HOME 복귀 회전 시작
//
// 현재 ID5 좌표 -> 저장해 둔 HOME(home_x, home_y) 좌표로
...
void ReturnHome_Start(void)
{
	...
	return_rotation_active = 1;
	return_rotation_finished = 0;

	return_forward_active = 0;
	return_forward_finished = 0;
...
// HOME 복귀 회전 제어
//
// Rotation_Control과 동일한 구조이며, 완료 시
// return_forward_active를 켜서 HOME 좌표까지 직진을 시작한다.
// ============================================================

void ReturnRotation_Control(void)
{
	float error;

	if (!return_rotation_active)
	{
		return;
	}
	...
	ANGLE_TOLERANCE)
	{
		...
		return_rotation_active = 0;
		return_rotation_finished = 1;

		return_forward_active = 1;
		return_forward_finished = 0;
...
// HOME 복귀 직진 제어
//
// TagForward_Control의 ID7 단계(좌표 일치 방식)와 동일한
// 구조로, 목표 좌표만 HOME(home_x, home_y)으로 바뀐다.
// ID5 좌표가 HOME과 일치하면 정지하고 전체 과정을 종료한다.
// ============================================================

void ReturnForward_Control(void)
{
	float error;

	if (!return_forward_active)
	{
		return;
	}
	...
	if (ID5_ReachedPosition(home_x, home_y))
	{
		Motor_Stop();

		return_forward_active = 0;
		return_forward_finished = 1;
...
void LCD_DisplayStatus(void)
{
	...
	if (return_forward_active)
	{
		...
		LCD_PrintUInt(home_x);

		LCD_Print(",");

		LCD_PrintUInt(home_y);
...
int main(void)
{
	...
	while (1)
	{
		...
		//
		// 회전 중 + 직진 중(직진 유지 보정용) +
		// HOME 복귀 회전/직진 중 모두 사용
		// ====================================================

		if (rotation_active ||
		tag_forward_active ||
		return_rotation_active ||
		return_forward_active)
		{
			MPU6050_UpdateAngle(
			gyro_z_offset
			);
		...
		// ====================================================
		// HOME 복귀 회전 / 직진 (ID7 완료 이후에만 활성화됨)
		// ====================================================

		ReturnRotation_Control();

		ReturnForward_Control();
